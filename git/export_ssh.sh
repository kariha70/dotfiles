...
read -r -p "Enter source directory [$DEFAULT_SRC_DIR]: " SRC_DIR
...
# Prompt for Files
read -r -p "Enter files to include [$DEFAULT_FILES]: " FILES
...
tar -C "$SRC_DIR" -czf - "${FILE_LIST[@]}" | openssl enc -e -aes-256-cbc -pbkdf2 -out "$OUTPUT_FILE"
